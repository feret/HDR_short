function main=main()
% command line: 
%      'KaDE' '-l' '6' '--count' 'Occurrences' 'ode.4.12.4.ka' '--output''ode.4.12.4' '--output-plot''../data/ode.4.12.4.data'
...
init(5)=t;
var(4)=init(4)/init(2); % q2
var(3)=init(3)/init(2); % q1
var(2)=init(2)+init(3)+init(4); % dimer
init(1)=6; % A(x[.],y[.])
var(1)=init(1); % mono
...
k(6)=4; % A(x[1],y[.]), A(x[.],y[1]) -> A(x[.],y[.]), A(x[.],y[.])
...
filename = '../data/ode.4.12.4.data';
fid = fopen (filename,'w');
fprintf(fid,'# KaDE -l 6 --count Occurrences ode.4.12.4.ka --output ode.4.12.4 --output-plot ../data/ode.4.12.4.data\n')
fprintf(fid,'# ')
fprintf(fid,'[T],')
fprintf(fid,'mono,')
fprintf(fid,'dimer,')
...
dydt(1)=dydt(1)+k(6)*y(4);
dydt(1)=dydt(1)+k(6)*y(4);

% rule    : A(x[.],y[.]), A(x[.],y[.]) -> A(x[1],y[.]), A(x[.],y[1])
% reaction: A(x[.],y[.]) + A(x[.],y[.]) -> A(x[.],y[1]).A(x[1],y[.])

dydt(1)=dydt(1)-1/2*k(5)*y(1)*y(1);
dydt(1)=dydt(1)-1/2*k(5)*y(1)*y(1);
dydt(4)=dydt(4)+1/2*k(5)*y(1)*y(1);
...
jacvar(4,2)=(0-(2*y(4)))/(y(2)**2);
jacvar(4,4)=y(2)/(y(2)**2);
jacvar(3,2)=(0-(2*y(3)))/(y(2)**2);
jacvar(3,3)=2*y(2)/(y(2)**2);
jacvar(2,2)=2;
jacvar(2,3)=2;
jacvar(2,4)=1;
jacvar(1,1)=1;
...
jac(1,1)=jac(1,1)-1/2*k(5)*y(1);
jac(1,1)=jac(1,1)-1/2*k(5)*y(1);
jac(1,1)=jac(1,1)-1/2*k(5)*y(1);
jac(1,1)=jac(1,1)-1/2*k(5)*y(1);
jac(4,1)=jac(4,1)+1/2*k(5)*y(1);
jac(4,1)=jac(4,1)+1/2*k(5)*y(1);
